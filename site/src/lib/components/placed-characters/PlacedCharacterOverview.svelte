<script lang="ts">
  import type { PlacedCharacterOverviewRow } from "$lib/server/read-models";

  let { rows }: { rows: PlacedCharacterOverviewRow[] } = $props();
</script>
...
{:else}
  <p class="text-muted-foreground mt-4">No placed characters found.</p>
{/if}

...
- **Close the descriptor type vocabulary.** `schemas/entity.schema.json:103-104` accepts any non-empty string as a field type, and both dispatchers fall through to TEXT for anything they do not recognise (`sql/ddl.ts:3-8`, `stages/emit-site-metadata.ts:5-12`). Three different author intentions rely on that fallback: `json` across many descriptors, `ref:asset[]` where an array is silently treated as a scalar ref, and `int` at `entities/item/variants/equipment.json:17`, which is a misspelling of `integer`. That last one is a confirmed live defect. `item_equipment.minimumSkill` ships as TEXT holding "0", "40" and "5", so a sort would place "40" before "5". Nothing reads it numerically today, which is the only reason it is invisible.

  Two guards are needed and they catch different failures. Closing the vocabulary in the schema catches a typo when it is authored. Making dispatch throw catches a type it was never taught. Reconcile the canonical spelling for `json` and the semantics of a ref array across every descriptor before closing anything, since the dispatchers currently claim support they do not have.

---
title: "Slice 4.5 — Items deterministic presentation closure"
type: spec
status: implemented
created: 2026-05-20
parent:
...
- **`item-category`** — `ItemCategory.cs:14-221`. Fields: `categoryName`, `iconRef`, `defaultItemIconRef`, `categoryColor: Color`, `showInAllCategory: bool`, `columns: CategoryColumn[]` (each column carries `label`, `iconRef`, `preferedWidth: float`, `flexibleWidth: float`, item/name/value booleans including `isAffectedBySkillRequirement` and `isAffectedByBrokenDurability`, `alignment`, `itemDataField`/`itemFunctionField` reflective names). Predicates: items `belongs_to_category` → item-category. `categoryColor` is the canonical source of the icon tint we currently drop. Item snapshots also carry `categoryName` beside `categoryRef` so category pages can fall back to name joins when Unity lookup GUIDs are unavailable.
...
- `ItemCategorySnapshot.cs` — id, categoryName, iconRef, defaultItemIconRef, categoryColor, showInAllCategory, columns: `ItemCategoryColumnSnapshot[]` (each: label, iconRef, preferedWidth: float, flexibleWidth: float, itemName, isItemIconAndCategory, itemValue, isAffectedBySkillRequirement, isAffectedByBrokenDurability, affectingRedColor, affectingIconsAfter, hideIfNegativeOne, alignment, itemDataField, itemFunctionField).

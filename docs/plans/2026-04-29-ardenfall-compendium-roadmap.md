...
**The name contract.** The extractor substituted the row id whenever `friendlyName` was empty, which shipped a route labelled `instances;portals;bd9b9562…`. Since an id-shaped label is indistinguishable downstream from an authored one, the extractor now emits `portalFriendlyNameMissing` and leaves the value null, `portals.friendly_name` is nullable, and the map layer supplies a visibly placeholder label. The column carries the game's own `friendlyName`, which holds authoring identifiers for 29 of its 32 values, so the name states its provenance and promises no display name. The canonical table records what the game contains, and only presentation fills the gap.
...
**Verification evidence:** live export on 2026-08-02 against Ardenfall Demo `0.0.10.91` produced 30 `leads_to` edges from 33 portals, matching the 30 that carry a connection, with zero unresolvable targets and `fatal` 0. The single portal with an empty value produced exactly one `portalFriendlyNameMissing` diagnostic and now renders as `Unnamed portal`. Driven in a browser against the fixture build: selecting a portal shows its destination, following it moves the selection and updates the deep link, and the reciprocal edge leads back.

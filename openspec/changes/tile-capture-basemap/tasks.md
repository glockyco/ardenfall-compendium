...
- [x] 1.5 Capture one interior cell, and settle the ceiling question. Record which option worked and what it changed.
- [x] 1.6 Capture one cell that has no authored scene, so the distant-cell fidelity difference is visible rather than assumed. Close the coverage decision in `design.md`.

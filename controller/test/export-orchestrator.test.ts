...
describe("exportCompendium", () => {
  ...
  it("refuses to run when preflight identifies a different game", async () => {
    ...
    await expect(
      ...
      /publication embargo.*expected Unity product name "Ardenfall Alpha".*reported "Vespera".*another install.*port collision/i,
    );
...
describe("validateSnapshot", () => {
  ...
  async function writeSnapshot(
      ...
      "placed-containers.json": JSON.stringify(
        {
          ...
          schemaVersion: 1,
          rows: emptyEntities.has("placed-container") ? [] : [{ id: "scene;cell_a;container-1" }],
        },
        null,
        2,
      ),
      "placed-items.json": JSON.stringify(
        {
          entityId: "placed-item",
          schemaVersion: 1,
          rows: emptyEntities.has("placed-item") ? [] : [{ id: "scene;cell_a;item-1" }],
        },
        null,
        2,
      ),
    ...
    const counts = {
      ...
      "placed-item":
        options.countOverrides?.["placed-item"] ?? (emptyEntities.has("placed-item") ? 0 : 1),
      "placed-container":

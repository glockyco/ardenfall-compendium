...
public sealed class CharacterRaceExtractionService : ICharacterRaceExtractionCache
{
    private readonly ICharacterRaceAssetSource _source;
    private readonly Dictionary<string, ExtractionState> _byRun = new();

    public CharacterRaceExtractionService(ICharacterRaceAssetSource source)
    {
        _source = source;
    }

    public IReadOnlyList<CharacterRaceSnapshotRow> GetOrExtract(CompendiumRun run) => GetState(run).Rows;
    ...
    private ExtractionState GetState(CompendiumRun run)
    {
        ...
        var extractor = new CharacterRaceExtractor(_source);
        var rows = new List<CharacterRaceSnapshotRow>();
    ...
    private sealed record ExtractionState(
        IReadOnlyList<CharacterRaceSnapshotRow> Rows,

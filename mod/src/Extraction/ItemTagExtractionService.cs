...
public sealed class ItemTagExtractionService : IItemTagExtractionCache
{
    private readonly IItemTagAssetSource _source;
    private readonly Dictionary<string, ExtractionState> _byRun = new();

    public ItemTagExtractionService(IItemTagAssetSource source)
    {
        _source = source;
    }

    public IReadOnlyList<ItemTagSnapshotRow> GetOrExtract(CompendiumRun run) => GetState(run).Rows;
    ...
    private ExtractionState GetState(CompendiumRun run)
    {
        ...
        var extractor = new ItemTagExtractor(_source);
        var rows = new List<ItemTagSnapshotRow>();
    ...
    private sealed record ExtractionState(
        IReadOnlyList<ItemTagSnapshotRow> Rows,

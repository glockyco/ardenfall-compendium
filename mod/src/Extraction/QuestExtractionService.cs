...
public sealed class QuestExtractionService : IQuestExtractionCache
{
    private readonly IQuestAssetSource _source;
    private readonly Dictionary<string, ExtractionState> _byRun = new();

    public QuestExtractionService(IQuestAssetSource source)
    {
        _source = source;
    }

    public IReadOnlyList<QuestSnapshotRow> GetOrExtract(CompendiumRun run) => GetState(run).Rows;
    ...
    private ExtractionState GetState(CompendiumRun run)
    {
        ...
        var extractor = new QuestExtractor(_source);
        var rows = new List<QuestSnapshotRow>();
    ...
    private sealed record ExtractionState(
        IReadOnlyList<QuestSnapshotRow> Rows,

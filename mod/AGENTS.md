...
Build HotRepl first, run `mod/scripts/copy-libs.sh <Ardenfall Managed dir> <HotRepl.Core output dir>` before `dotnet build mod/ArdenfallCompendium.csproj`, and then deploy through `bun run hotrepl:deploy`. Deploy ships every DLL from `HOTREPL_BEPINEX_OUT` into `BepInEx/plugins/HotRepl/` and requires at least `HotRepl.BepInEx.dll`, `HotRepl.Core.dll`, and `mcs.dll` to be present in that source directory. `Namotion.Reflection.dll` is internalized into `HotRepl.Core.dll`; never deploy it as a sidecar.

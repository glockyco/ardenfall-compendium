...
- `entity_nodes.has_page` means one thing: the entity has a page. A consumer that wants a stable identity, such as a map marker's short id or an edge target that renders no section, must not test it. A portal is the standing example, since it carries an identity and a map route with no page.
